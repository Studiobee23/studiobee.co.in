...
- **`/performance` page** (new: `src/app/(app)/performance/page.tsx` + `performance-client.tsx`):
  ...
  - `manager`/`admin`: table of all `role = 'employee'` profiles with running scores (sortable) — peers scoring peers doesn't make sense, so admin/manager never see admin/manager rows here; click through to an employee's history; "Log event" control enabled only where permitted (manager: `employee.manager_id === self`; admin: always).
  - `super_admin`: same table but expanded to `role in ('admin', 'manager', 'employee')` — super_admin sits above the whole hierarchy, so its scoring power extends to everyone below it, not just rank-and-file employees. (Revision: v1 scoped this to employees only; widened after initial implementation once "everyone comes under super admin" was clarified.) Plus two extra tabs — **Point Reasons** (`point-reasons-tab.tsx`, new) and **Profit Split** (`profit-split-tab.tsx`, adapted from the current `profit-split-client.tsx`).
...
- **Accepted trade-off (partially hardened in migration `0033`):** the "logger updates own" `point_events` policy now has a `with check` requiring the target employee's `manager_id` still match the caller — this closes the worst vector (a manager reassigning their own logged row to an employee outside their reports). It does *not* pin `points`/`reason_id` to their original values, so a manager could still tamper with those on a row they logged (the app's `updatePointEvent` action only ever exposes `note`, but RLS alone doesn't enforce that). Severity is low — still scoped to rows they created, no privilege escalation, no ability to reassign outside their scope — so full column-immutability (a trigger) is deferred unless it matters in practice.
